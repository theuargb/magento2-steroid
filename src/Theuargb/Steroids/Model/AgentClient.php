...
use Theuargb\Steroids\Agent\HealerAgent;
use Theuargb\Steroids\Agent\FallbackAgent;
use Theuargb\Steroids\Agent\Result\HealResult;
use Theuargb\Steroids\Agent\Result\FallbackResult;
use Theuargb\Steroids\Agent\Observer\AgentLogger;
...
class AgentClient implements AgentClientInterface
{
    ...
    public function requestHealing(array $context, int $timeout): HealResult
    {
        ...
        // Enrich context with healer prompt if present
        $healerPrompt = $context['healer_prompt'] ?? null;
        if ($healerPrompt) {
            $contextStr = "=== ADMIN INSTRUCTIONS FOR THIS URL ===\n"
                . $healerPrompt . "\n"
        ...
        try {
            ...
            $agent->configure([
                ...
                'allow_file_writes' => $this->config->isFileWriteAllowed(),
