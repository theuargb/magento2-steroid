...
use Theuargb\Steroids\Api\Data\HomepageSnapshotInterface;
use Theuargb\Steroids\Model\HomepageSnapshotFactory;
use Theuargb\Steroids\Model\ResourceModel\HomepageSnapshot as SnapshotResource;
use Theuargb\Steroids\Model\ResourceModel\HomepageSnapshot\CollectionFactory as SnapshotCollectionFactory;
...
class HomepageSnapshotManager
{
    ...
    public function capture(): ?HomepageSnapshotInterface
    {
        try {
            ...
            $curl->addHeader('User-Agent', 'Steroids-SnapshotBot/1.0');
            $curl->get($baseUrl);
            ...
            if ($statusCode !== 200 || empty($body)) {
                $this->logger->warning(
                    '[Steroids] Homepage snapshot capture got HTTP ' . $statusCode
                );
                return null;
            }
            ...
            $this->logger->info('[Steroids] Homepage snapshot captured for store ' . $storeId);
            return $snapshot;
        } catch (\Throwable $e) {
            $this->logger->error('[Steroids] Snapshot capture failed: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function getLatestSnapshot(): ?HomepageSnapshotInterface
    {
        try {
            ...
            $this->logger->error('[Steroids] Failed to retrieve snapshot: ' . $e->getMessage());
            return null;
        }
    }
        ...
        if (preg_match_all('/<link[^>]+rel=["\']stylesheet["\'][^>]*href=["\']([^"\']+)["\'][^>]*>/i', $html, $linkMatches)) {
            foreach ($linkMatches[1] as $href) {
                try {
                    ...
                    $stylesheetCurl->addHeader('User-Agent', 'Steroids-SnapshotBot/1.0');
                    ...
                    $this->logger->debug(
                        '[Steroids] Failed to fetch stylesheet: ' . $href . ' - ' . $e->getMessage()
                    );
                }
            }
        }
